...
assert(html.includes('recognition.continuous = true'), 'recognition must stay continuous so a breath does not end the question');
assert(!html.includes('recognition.continuous = !!(handsFreeEnabled && !tapToTalkTurn)'), 'tap-to-talk must not end the utterance on short silence');
assert(!html.includes('/pause|hold|wait|longer|thinking|breath|one sec|sec/'), 'old substring pause regex must be gone');
assert(!html.includes('isFinalResult ? 450 : 850'), '450ms/850ms commit is too short for a natural pause');
assert(html.includes('return 2300'), 'auto-send waits ~2.3s of real silence');
assert(html.includes('scheduleVoiceCommit()'), 'more speech must reset the silence timer');
assert(!html.includes('shouldFlushTap'), 'onend must not flush-send a breath-ended utterance');
assert(html.includes('Tapping the mic while a transcript is showing means "send this now"'), 'second mic tap still sends immediately');

// onend after a breath: keep the fragment, do not send, keep listening.
function flushOnEnd({ leftover, tapToTalkTurn, isAwaitingResponse, alreadySent }) {
  if (alreadySent || isAwaitingResponse) return { send: null, restart: false };
  if (tapToTalkTurn && leftover && leftover.trim().length > 1) {
    return { send: null, keep: leftover.trim(), restart: true };
  }
  return { send: null, restart: false };
}
assert(flushOnEnd({ leftover: 'tell me about John 1:14', tapToTalkTurn: true, isAwaitingResponse: false, alreadySent: false }).send === null, 'onend must not send a breath-ended tap-to-talk fragment');
assert(flushOnEnd({ leftover: 'tell me about John 1:14', tapToTalkTurn: true, isAwaitingResponse: false, alreadySent: false }).restart === true, 'onend must keep listening so the question can continue');
assert(flushOnEnd({ leftover: 'tell me about John 1:14', tapToTalkTurn: true, isAwaitingResponse: true, alreadySent: false }).restart === false, 'do not restart while chat is in flight');
assert(flushOnEnd({ leftover: '', tapToTalkTurn: true, isAwaitingResponse: false, alreadySent: false }).send === null, 'empty leftover does not send');
...
eval(extractFn('stripSpokenFullStop'));
...
assert(stripSpokenFullStop('divorce.') === 'divorce', 'trailing period is not spoken');
assert(stripSpokenFullStop('Matthew 19 9 is about divorce.') === 'Matthew 19 9 is about divorce', 'British voices must not receive a final full stop');
assert(stripSpokenFullStop('See 19.9') === 'See 19.9', 'decimals are not treated as a sentence stop');

const firstFromClean = takeFirstSpeechChunk(multiSpoken);
assert(firstFromClean === 'Matthew 19 9 is about divorce', 'first chunk is the first sentence with no spoken full stop');
assert(!firstFromClean.includes('husband'), 'do not start speaking the second sentence in the first chunk');
assert(!/\.$/.test(firstFromClean), 'spoken chunks must not end with a period');

const firstShort = takeFirstSpeechChunk('Matthew 19 9 is about divorce.');
assert(firstShort === 'Matthew 19 9 is about divorce', 'a single sentence is one chunk without a trailing period');
...
assert(cleaned.startsWith('Matthew 19 9 is about divorce and remarriage.'), 'cleaned text still uses periods to split sentences');
const rest = cleaned.slice('Matthew 19 9 is about divorce and remarriage.'.length).replace(/^\s+/, '');
...
assert(more[0] === 'Matthew 19 9 is about divorce and remarriage', 'each queued chunk is a full sentence without a spoken full stop');
assert(more[more.length - 1] === 'The Greek word is porneia', 'last chunk is the last sentence without a trailing period');
assert(more.every((c) => !/\.$/.test(c)), 'no queued chunk ends with a period a UK voice would read as full stop');

let longPauseUntil = 0;
let pendingTranscript = null;
eval(extractFn('voiceCommitDelayMs'));
eval(extractFn('handsFreeCommitDelayMs'));
eval(extractFn('rememberVoiceTranscript'));

assert(voiceCommitDelayMs() >= 2000 && voiceCommitDelayMs() <= 2500, 'natural pause must wait about 2 to 2.5s');
assert(handsFreeCommitDelayMs() === voiceCommitDelayMs(), 'hands-free and tap-to-talk share the same silence window');
longPauseUntil = Date.now() + 8000;
assert(voiceCommitDelayMs() === 60000, 'explicit pause command can still hold longer');
longPauseUntil = 0;

pendingTranscript = null;
rememberVoiceTranscript('tell me about Matthew');
rememberVoiceTranscript('19 9 and divorce');
assert(pendingTranscript === 'tell me about Matthew 19 9 and divorce', 'more speech after a pause appends to the same question');
rememberVoiceTranscript('tell me about Matthew 19 9 and divorce in the Greek');
assert(pendingTranscript === 'tell me about Matthew 19 9 and divorce in the Greek', 'a growing same-session transcript replaces, not duplicates');

const scheduleSrc = extractFn('scheduleVoiceCommit');
assert(scheduleSrc.includes('if (commitTimeout) clearTimeout(commitTimeout)'), 'more speech must reset the 2.3s silence timer');
assert(scheduleSrc.includes('voiceCommitDelayMs()'), 'auto-send uses the shared 2.3s silence window');

// Second mic tap (or Send) commits immediately — same leftover path as consumeAndSendVoice.
function tapMicSendsNow({ isListening, leftover }) {
  if (!isListening) return null;
  const t = (leftover || '').trim();
  return t.length > 1 ? t : null;
}
assert(tapMicSendsNow({ isListening: true, leftover: 'what is the meaning of John' }) === 'what is the meaning of John', 'second mic tap sends the full pending question now');
assert(tapMicSendsNow({ isListening: false, leftover: 'what is the meaning of John' }) === null, 'mic tap while idle starts listening, it does not send');
const micClickSrc = html.slice(html.indexOf('micBtn.addEventListener("click"'), html.indexOf('sendBtn.addEventListener("click"'));
assert(micClickSrc.includes('consumeAndSendVoice(leftover)'), 'mic click while listening must call consumeAndSendVoice');
assert(extractFn('consumeAndSendVoice').includes('clearTimeout(commitTimeout)'), 'immediate send cancels the silence timer');

const sw = fs.readFileSync(path.join(__dirname, '../public/sw.js'), 'utf8');
assert(sw.includes("CACHE_VERSION = 'wic-pwa-67'"), 'service worker cache must bump so production drops a stale shell');
