...
async function main() {
  ...
  assert(!recSrc.includes('Control Center'), 'must not teach Control Center on iPhone');
  assert(!recSrc.includes('session-record-ios-guide'), 'must not inject an iOS guide dialog');
  assert(!/Screen Recording/.test(recSrc), 'must not name iOS Screen Recording in a guide');
  ...
  assert(
    ...
    'iPhone Record button is disabled'
  );
  ...
  assert(iosMount.btn.disabled === true, 'mount disables #session-record-btn on iPhone');
  assert(
    iosMount.btn.title === 'Recording not supported. Use Chrome on a computer.',
    'iOS mount sets quiet unsupported title, not a Record session (iPhone) label'
  );
  assert(iosMount.btnListeners.length > 0, 'Record click is still wired');
  await iosMount.btnListeners[0]({ preventDefault() {} });
  assert(iosMount.doc.getElementById('session-record-ios-guide') == null, 'iPhone Record tap must not open a guide popup');
  assert(iosAlerted === false, 'iPhone Record tap must not show an instructional alert/modal');
