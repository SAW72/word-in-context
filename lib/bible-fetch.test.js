...
(async () => {
  ...
  if (!/πορνείᾳ/.test(grc.text)) {
    throw new Error(`Matthew 19:9 SBL missing πορνείᾳ: ${grc.text}`);
  }
  ...
  console.log(`bible-fetch.test.js: Psalm 136 WLC=${psalm} vv; John 1 BSB=${johnEn} vv SBL=${johnGrc} vv; Matt 19:9 SBL+BSB ok`);
})().catch((err) => {
  console.error(err);
  process.exit(1);
});
