...
const { flattenVerseContent, fetchBiblePassage, fetchStudyGrounding } = require('./bible-fetch');
const {
...
(async () => {
  ...
  for (const ref of ['Matthew 19:9', '1 Corinthians 7:15']) {
    ...
    groundingPassages.push(live);
  }

  // Success test: the [ORIGINAL GREEK TEXT] block John sees must carry live porneia
  // from SBL Matthew 19:9, and live Greek for 1 Corinthians 7:15.
  const originalBlocks = formatGroundingBlocks(groundingPassages);
  if (!hasOriginalGreekBlock(originalBlocks, 'Matthew 19:9')) {
    throw new Error('Matthew 19:9 missing [ORIGINAL GREEK TEXT] block');
  }
  if (!PORNEIA_LIVE_RE.test(originalBlocks)) {
    throw new Error(`Matthew 19:9 original block missing πορνεία / πορνείας: ${originalBlocks}`);
  }
  if (!hasOriginalGreekBlock(originalBlocks, '1 Corinthians 7:15')) {
    throw new Error('1 Corinthians 7:15 missing [ORIGINAL GREEK TEXT] block');
  }
  if (!/δεδούλωται/.test(originalBlocks) || !/χωρίζεται/.test(originalBlocks)) {
    throw new Error(`1 Corinthians 7:15 original block missing live Greek: ${originalBlocks}`);
  }

  const studyMatt = await fetchStudyGrounding('Matthew 19:9', { quiet: true });
  if (!studyMatt || !studyMatt.original || !PORNEIA_LIVE_RE.test(studyMatt.original.text)) {
    throw new Error('fetchStudyGrounding(Matthew 19:9) must return live SBL πορνεία');
  }
  const studyPsalm = await fetchStudyGrounding('Psalm 136:1', { quiet: true });
  if (!studyPsalm || !studyPsalm.original || !/חֶסֶד|חַסְד|חסד/.test(studyPsalm.original.text)) {
    throw new Error('fetchStudyGrounding(Psalm 136:1) must return live WLC hesed form');
  }
  const studyMiss = await fetchStudyGrounding('Matthew 99:1', { quiet: true });
  if (studyMiss !== null) {
    throw new Error('fetchStudyGrounding must return null when original fetch is empty');
  }

  console.log(`bible-fetch.test.js: Psalm 136 WLC=${psalm} vv; John 1 BSB=${johnEn} vv SBL=${johnGrc} vv; Matt 19:9 + 1 Cor 7:15 ORIGINAL blocks ok`);
})().catch((err) => {
  console.error(err);
  process.exit(1);
});
